...
class Sum(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        if (in_data[0].size(2) - in_data[1].size(2)) != 0:
            small_in_id, large_in_id = (0, 1) if in_data[0].size(2) < in_data[1].size(2) else (1, 0)
            pool_num = int(math.log2(in_data[large_in_id].size(2)) - math.log2(in_data[small_in_id].size(2)))
...
class SepConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride=1, affine=False):
        ...
        self.op = nn.Sequential(
            ...
            nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=stride, padding=pad_size, groups=C_in, bias=False),
            nn.Conv2d(C_in, C_in, kernel_size=1, padding=0, bias=False),
            nn.BatchNorm2d(C_in, affine=affine),
            nn.ReLU(inplace=False),
            nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=1, padding=pad_size, groups=C_in, bias=False),
...
class DilConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, dilation=2, stride=1, affine=False):
        ...
        self.op = nn.Sequential(
            nn.ReLU(inplace=False),
            nn.Conv2d(C_in, C_in, kernel_size=kernel_size, stride=stride, padding=pad_size, dilation=dilation, groups=C_in, bias=False),
            nn.Conv2d(C_in, C_out, kernel_size=1, padding=0, bias=False),
            nn.BatchNorm2d(C_out, affine=affine),)
...
class CGP2CNN(nn.Module):
    def __init__(self, cgp, in_channel, n_class, img_size):
        ...
        for name, in1 in self.cgp:
            if name == 'input' in name:
                ...
                self.encode.append(nn.Linear(self.channel_num[in1], n_class))
                ...
                if down == 'S':
                    if func == 'SepBlock':
                        ...
                        self.encode.append(SepConv(self.channel_num[in1], out_size, kernel))
                    elif func == 'DilBlock':
                        self.channel_num[i] = out_size
                        self.size[i] = self.size[in1]
                        self.encode.append(DilConv(self.channel_num[in1], out_size, kernel))
                    elif func == 'ResBlock':
