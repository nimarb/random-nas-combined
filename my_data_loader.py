...
def get_train_valid_loader(data_dir, batch_size, augment, random_seed, valid_size=0.1, shuffle=True, show_sample=False, num_workers=4, pin_memory=False,
    ...
    train_transform, valid_transform = utils._data_transforms_cifar10(False, 16)
    ...
    balanced_train_indices = np.load('./sample_indices/training_sample_list_%d.npy' % (data_num))
    balanced_train_sampler = SubsetRandomSampler(balanced_train_indices.tolist())
    balanced_valid_indices = np.load('./sample_indices/valid_sample_list_%d.npy' % (data_num))
    balanced_valid_sampler = SubsetRandomSampler(balanced_valid_indices.tolist())
...
def get_train_valid_loader_tinyimagenet(data_dir, batch_size, augment, random_seed, valid_size=0.1, shuffle=True, show_sample=False, num_workers=4, pin_memory=False,
    ...
    # load the dataset
    train_dataset = torchvision.datasets.ImageFolder(root='/home/suganuma/dataset/tiny-imagenet-200/train', transform=transform_train)
    testset = torchvision.datasets.ImageFolder(root='/home/suganuma/dataset/tiny-imagenet-200/train', transform=transform_test)
